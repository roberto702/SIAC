<?php
		...
		if(!empty($_GET['estado'])){
			...
			$cans=mysql_query("SELECT * FROM salones WHERE estado='s' and id='$rut'");
			if($dat=mysql_fetch_array($cans)){
				$xSQL="Update salones Set estado='n' Where id='$rut'";
				mysql_query($xSQL);
				header('location:salones.php');
			}else{
				$xSQL="Update salones Set estado='s' Where id='$rut'";
				mysql_query($xSQL);
				header('location:salones.php');
			}
		}
